...
export const getLanguage = (supported, userNavigator = navigator, languageDefault = process.env.INTERFACE_LANGUAGE_DEFAULT, useClient = process.env.INTERFACE_USE_CLIENT) => {
  ...
  const userLanguage = userNavigator.language || userNavigator.userLanguage

  // TODO need to handle cases where this isn't an array
  if (!supported) {
    return false
  }
  ...
  // First check if userLanguage is supported localized
