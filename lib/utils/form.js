import { getLabel } from './content'; // Returns an object of form values from a form
...
export const getFormData = form => [].reduce.call(form.elements, (data, element) => {
  ...
  if (element.name && element.value && (!['checkbox', 'radio'].includes(element.type) || element.checked)) {
    if (element.type === 'checkbox') {
      ...
      data[element.name] = element.value || ''; // TODO re-examine, returns an array
...
export const getFormConfig = (id, language, forms = {}) => {
  ...
  return Object.assign({
    ...
    action: forms[id].action.replace('%%API_URL%%', process.env.API_URL)
  }, form);
};
