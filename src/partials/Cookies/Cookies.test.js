...
it('should pass style to content', () => {
  ...
  const wrapper = shallow(<Cookies style={testStyle} />)
  expect(wrapper.props().style).toEqual(testStyle)
})

it('should contain default className and prop className', () => {
  const wrapper = shallow(<Cookies className='test-class-name' />)
...
it('should contain a title when passed content with "page title"', () => {
  const text = 'Title'
  const wrapper = shallow(<Cookies content={{'page title': {'en': text}}} />)
  const element = <Content className='tc-cookies__title' headerType='headline4' header={text} />
  expect(wrapper).toContainReact(element)
})

it('should contain an introduction when passed content with "introduction"', () => {
  const text = 'Intro'
  const wrapper = shallow(<Cookies content={{'introduction': {'en': text}}} />)
  const element = <Content className='tc-cookies__text' body={text} />
  expect(wrapper).toContainReact(element)
})

it('should contain a cookie title when passed content with "cookie title"', () => {
  const text = 'Cookies'
  const wrapper = shallow(<Cookies content={{'cookie title': {'en': text}}} />)
  const element = <Content className='tc-cookies__header' headerType='headline6' header={text} />
  expect(wrapper).toContainReact(element)
})
...
it('should contain a cookies table when passed content with "cookies"', () => {
  ...
  const wrapper = shallow(<Cookies content={{'cookies': data}} />)
  const element = (
    <Table 
      wrap
...
it('should contain a storage title when passed content with "storage title"', () => {
  const text = 'Storage'
  const wrapper = shallow(<Cookies content={{'storage title': {'en': text}}} />)
  const element = <Content className='tc-cookies__header' headerType='headline6' header={text} />
  expect(wrapper).toContainReact(element)
})
...
it('should contain a storage table when passed content with "storage"', () => {
  ...
  const wrapper = shallow(<Cookies content={{'storage': data}} />)
  const element = (
    <Table 
      wrap
      className='tc-cookies__table'
...
it('should contain an statement on privacy when passed content with "privacy"', () => {
  const text = 'Privacy'
  const wrapper = shallow(<Cookies content={{'privacy': {'en': text}}} />)
  const element = <Content className='tc-cookies__text' body={text} />
  expect(wrapper).toContainReact(element)
})
