...
it('should contain menu items for each item in languages', () => {
  ...
  const wrapper = shallow(<LanguageMenu languages={['en', 'fr']} onClick={onClick} />, {skipProps: ['onClick']})
  expect(wrapper.find(MenuItem).length).toEqual(2)
})

it('should display language and onclick handler in menu items', () => {
  const onClick = jest.fn()
  const wrapper = shallow(<LanguageMenu languages={['en']} onClick={onClick} />, {skipProps: ['onClick']})
...
it('should set a regular class and an active class when language is active', () => {
  const onClick = jest.fn()
  const wrapper = shallow(<LanguageMenu language='en' languages={['en']} onClick={onClick} />, {skipProps: ['onClick']})
